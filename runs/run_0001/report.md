# Site analysis — Ashburn, Loudoun County VA

**Loudoun County, Virginia, United States** · `39.0437, -77.4875` · AOI radius 10 km
**Market:** PJM
**Run:** `run_0001` · 2026-09-02T14:00:04Z
...
Not yet — and the reason is missing evidence, not a bad site. 8 knockout checks cannot be evaluated at all because the factor each one reads is unmeasured; the regulatory domain has no data at all and drops out of the score; 18 unmeasured factors could each move the verdict across a threshold on its own; only 25% of the decision weight is measured. Profitability in particular is unanswerable here: 5 of 5 cost inputs are unmeasured (Modeled 10-year TCO, Industrial retail power price, Large-load tariff availability, Land acquisition cost, Tax incentive package), so the score is a suitability screen, not a return. The verification queue is ordered by how much of the decision each answer settles.

*For profile `hyperscale_training`. Score 60 ± 18, confidence 0.20, 25% of decision weight measured.*

> **1 domain(s) contributed nothing to this score** — regulatory. The headline is a weighted mean over the domains that had data, not over the whole model.
...
|---|---|---|---|
| **FATAL** | Nothing measured in regulatory — all 7 factors dark | 14 pts | Export-control counsel |
| **FATAL** | Substation available capacity is unmeasured, and Credible power path to target load depends on it | 8 pts | Serving utility transmission planning |
| **FATAL** | Interconnection queue time to energization is unmeasured, and Credible power path to target load depends on it | 8 pts | ISO/RTO interconnection queue, then the utility |
| **FATAL** | Organized opposition risk is unmeasured, and No active data center moratorium depends on it | 7 pts | Public record — planning commission and board minutes |
| **FATAL** | Baseline water stress is unmeasured, and Water viability for the assumed cooling architecture depends on it | 6 pts | Water utility or basin regulator |
...
| **MAJOR** | Modeled 10-year TCO is unmeasured | 8 pts | Owner's cost consultant |
| **MAJOR** | Construction labor availability is unmeasured | 8 pts | Owner's cost consultant / EPC |
| **MAJOR** | Critical equipment lead time is unmeasured | 8 pts | OEM / EPC procurement |
| **MAJOR** | Industrial retail power price is unmeasured | 7 pts | Utility rate desk or wholesale market data provider |
| **MAJOR** | Construction cost index is unmeasured | 7 pts | Owner's cost consultant / EPC |
| | *… 14 more* | | |

**Nothing measured in regulatory — all 7 factors dark**

This domain carries weight 8 of 57 in this profile and has no values at all, so it is excluded from the score: the headline number is a weighted mean over the 7 domains that do have data, not over all 8. It also holds the input to 1 knockout check(s), which are therefore being decided blind.

- *Closes with:* Dispatch the regulatory analyst. Highest-swing factor in the domain is Accelerator export control exposure at 14 points.
...
25 factor(s) can move the score across a verdict threshold on their own. Each row is the real scorer re-run with that one factor resolved at its best and worst plausible value, everything else held.

| Factor | State | Swing | Worst case | Best case |
|---|---|---|---|---|
| Accelerator export control exposure <br>`reg.export_controls` | unmeasured | ±14 | viable | strong-candidate |
| Permitting timeline <br>`reg.permitting_timeline` | unmeasured | ±14 | viable | strong-candidate |
| Environmental review burden <br>`reg.environmental_review` | unmeasured | ±14 | viable | strong-candidate |
| Tax incentive package <br>`reg.tax_incentives` | unmeasured | ±14 | viable | strong-candidate |
| Generator air permitting <br>`reg.air_permit` | unmeasured | ±14 | viable | strong-candidate |
| Jurisdictional and political stability <br>`reg.jurisdiction_stability` | unmeasured | ±14 | viable | strong-candidate |
| Data sovereignty and cross-border regime <br>`reg.data_sovereignty` | unmeasured | ±14 | viable | strong-candidate |
| Port and rail access <br>`eco.port_rail_access` | tier B | ±12 | viable | strong-candidate |
| Transmission line proximity <br>`pwr.transmission_proximity` | tier A | ±11 | viable | strong-candidate |
| On-site generation potential <br>`pwr.onsite_generation_potential` | tier B | ±10 | viable | strong-candidate |
| Wet-bulb temperature profile <br>`wtr.wetbulb_profile` | tier A | ±9 | viable | strong-candidate |
| Dry-bulb temperature profile and free-cooling hours <br>`clm.dry_bulb_profile` | tier A | ±8 | viable | strong-candidate |
...
1. **[gate]** **Can the intended compute legally be installed and operated here?**
   - `reg.export_controls` · 14 points at risk · 3–10 weeks
   - **Ask:** Export-control counsel
...
2. **[gate]** **How much spare capacity (MVA) exists at the nearest suitable substation?**
   - `pwr.substation_headroom` · 8 points at risk · 8–26 weeks
...
3. **[gate]** **Realistic years from application to energization for target load.**
   - `pwr.interconnect_queue_time` · 8 points at risk · 4–12 weeks
...
4. **[gate]** **Is there active, organized opposition to data centers in this jurisdiction?**
   - `com.opposition_risk` · 7 points at risk · 2–6 weeks
...
5. **[gate]** **What fraction of available renewable supply is already withdrawn in this basin?**
   - `wtr.basin_stress` · 6 points at risk · 8–26 weeks
...
|---|---|---|---|---|
| `hyperscale_training` | **viable** *(capped by gate.compute_legally_deployable)* | 60 ± 18 | 0.20 | 25% |
| `inference_edge` | **viable** *(capped by gate.compute_legally_deployable)* | 60 ± 17 | 0.21 | 26% |
| `retrofit_colo` | **viable** *(capped by gate.compute_legally_deployable)* | 60 ± 18 | 0.20 | 24% |
...
- Entitlement path exists — Entitlement status: by_right_industrial.
...
| Contiguous developable area <br>`lnd.contiguous_area` | 1256.6 | hectares | 100 | 9 | C (researched) | `osm_overpass` | 2026-09-02 (2d) |
| Transmission line proximity <br>`pwr.transmission_proximity` | 0.97 | km | 98 | 9 | A (machine API) | `osm_overpass` | 2026-09-02 (2d) |
...
| Zoning and entitlement status <br>`lnd.zoning_status` | by_right_industrial | category | 100 | 8 | B (bulk/structured) | `us_county_gis` | 2026-09-02 (2d) |
| Large-load tariff availability <br>`pwr.large_load_tariff` | — | category | — | 8 | — (unmeasured) | — | — |
| On-site generation potential <br>`pwr.onsite_generation_potential` | 74.7 | index_0_100 | 75 | 8 | B (bulk/structured) | `composite` | 2026-09-02 (2d) |
...
| Dry-bulb temperature profile and free-cooling hours <br>`clm.dry_bulb_profile` | 7271.5 | free_cooling_hours_per_year | 90 | 7 | A (machine API) | `open_meteo_archive` | 2026-09-02 (2d) |
| Residential proximity and noise exposure <br>`com.residential_proximity` | 1133 | dwellings_within_1km | 18 | 7 | A (machine API) | `osm_overpass` | 2026-09-02 (2d) |
| Long-haul fiber proximity <br>`cnx.longhaul_fiber_proximity` | 2.06 | km | 92 | 7 | C (researched) | `osm_overpass` | 2026-09-02 (2d) |
...
| Wet-bulb temperature profile <br>`wtr.wetbulb_profile` | 230.2 | hours_above_24C_wetbulb | 84 | 7 | A (machine API) | `open_meteo_archive` | 2026-09-02 (2d) |
...
| Reclaimed water availability <br>`wtr.reclaimed_availability` | 3.04 | km_to_wwtp | 88 | 6 | B (bulk/structured) | `osm_overpass` | 2026-09-02 (2d) |
| Extreme heat trend to 2050 <br>`clm.extreme_heat_trend` | 1.67 | delta_C_design_day_2050 | 63 | 5 | D (modeled) | `open_meteo_archive` | 2026-09-02 (2d) |
...
| Carrier availability <br>`cnx.carrier_count` | 60 | count_carriers | 100 | 4 | B (bulk/structured) | `peeringdb` | 2026-09-02 (2d) |
| Port and rail access <br>`eco.port_rail_access` | 20.37 | km_to_capable_port_or_rail | 93 | 4 | B (bulk/structured) | `osm_overpass` | 2026-09-02 (2d) |
...
| Internet exchange and cloud on-ramp proximity <br>`cnx.ixp_proximity` | 3.08 | km | 98 | 3 | A (machine API) | `peeringdb` | 2026-09-02 (2d) |
...
- **Substation available capacity** (`pwr.substation_headroom`) — OSM does not publish substation capacity. Found 313 substation(s) within 40 km (174 at >=110 kV); nearest qualifying is Ashburn Substation at 1.26 km, 230.0 kV. Headroom must be obtained from the utility's IRP or transmission planning study.
...
**Sources used:** `composite`, `open_meteo_archive`, `osm_overpass`, `peeringdb`, `us_county_gis`
