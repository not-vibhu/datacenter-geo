# Site analysis — Abilene, Taylor County TX (Stargate area)

**Taylor County, Texas, United States** · `32.4200, -99.8500` · AOI radius 10 km
**Market:** ERCOT
**Run:** `run_0002` · 2026-09-02T14:10:43Z
...
Not yet — and the reason is missing evidence, not a bad site. 8 knockout checks cannot be evaluated at all because the factor each one reads is unmeasured; the regulatory, community domains have no data at all and drop out of the score; 17 unmeasured factors could each move the verdict across a threshold on its own; only 19% of the decision weight is measured. Profitability in particular is unanswerable here: 5 of 5 cost inputs are unmeasured (Modeled 10-year TCO, Industrial retail power price, Large-load tariff availability, Land acquisition cost, Tax incentive package), so the score is a suitability screen, not a return. The verification queue is ordered by how much of the decision each answer settles.

*For profile `hyperscale_training`. Score 60 ± 19, confidence 0.14, 19% of decision weight measured.*

> **3 domain(s) contributed nothing to this score** — regulatory, community, economics. The headline is a weighted mean over the domains that had data, not over the whole model.
...
|---|---|---|---|
| **FATAL** | Nothing measured in regulatory — all 7 factors dark | 18 pts | Export-control counsel |
| **FATAL** | Nothing measured in community — all 4 factors dark | 14 pts | Public record — planning commission and board minutes |
| **FATAL** | Substation available capacity is unmeasured, and Credible power path to target load depends on it | 10 pts | Serving utility transmission planning |
| **FATAL** | Interconnection queue time to energization is unmeasured, and Credible power path to target load depends on it | 10 pts | ISO/RTO interconnection queue, then the utility |
| **FATAL** | Baseline water stress is unmeasured, and Water viability for the assumed cooling architecture depends on it | 8 pts | Water utility or basin regulator |
| **FATAL** | Riverine and pluvial flood exposure is unmeasured, and Flood exclusion depends on it | 8 pts | Licensed civil / geotechnical engineer |
| **FATAL** | Water rights and withdrawal permitting is unmeasured, and Water viability for the assumed cooling architecture depends on it | 8 pts | State/provincial water regulator |
| **FATAL** | Zoning and entitlement status is unmeasured, and Entitlement path exists depends on it | 6 pts | County/municipal planning department |
...
| **FATAL** | Protected area and habitat overlap is unmeasured, and Protected area exclusion depends on it | 5 pts | County or municipal planning department |
| **MAJOR** | Nothing measured in economics — all 6 factors dark | 14 pts | Owner's cost consultant |
| **MINOR** | Long-haul fiber proximity rests on a model, not a measurement | 9 pts | Carrier account teams |
| **MINOR** | Industrial retail power price is unmeasured | 9 pts | Utility rate desk or wholesale market data provider |
| **MINOR** | Large-load tariff availability is unmeasured | 9 pts | Utility rate desk / regulator tariff filings |
| | *… 18 more* | | |

**Nothing measured in regulatory — all 7 factors dark**

This domain carries weight 8 of 57 in this profile and has no values at all, so it is excluded from the score: the headline number is a weighted mean over the 5 domains that do have data, not over all 8. It also holds the input to 1 knockout check(s), which are therefore being decided blind.

- *Closes with:* Dispatch the regulatory analyst. Highest-swing factor in the domain is Accelerator export control exposure at 18 points.
- *Ask:* Export-control counsel

**Nothing measured in community — all 4 factors dark**

This domain carries weight 6 of 57 in this profile and has no values at all, so it is excluded from the score: the headline number is a weighted mean over the 5 domains that do have data, not over all 8. It also holds the input to 1 knockout check(s), which are therefore being decided blind.

- *Closes with:* Dispatch the community analyst. Highest-swing factor in the domain is Organized opposition risk at 14 points.
...
21 factor(s) can move the score across a verdict threshold on their own. Each row is the real scorer re-run with that one factor resolved at its best and worst plausible value, everything else held.

| Factor | State | Swing | Worst case | Best case |
|---|---|---|---|---|
| Accelerator export control exposure <br>`reg.export_controls` | unmeasured | ±18 | conditional | strong-candidate |
| Permitting timeline <br>`reg.permitting_timeline` | unmeasured | ±18 | conditional | strong-candidate |
| Environmental review burden <br>`reg.environmental_review` | unmeasured | ±18 | conditional | strong-candidate |
| Tax incentive package <br>`reg.tax_incentives` | unmeasured | ±18 | conditional | strong-candidate |
| Generator air permitting <br>`reg.air_permit` | unmeasured | ±18 | conditional | strong-candidate |
| Jurisdictional and political stability <br>`reg.jurisdiction_stability` | unmeasured | ±18 | conditional | strong-candidate |
| Data sovereignty and cross-border regime <br>`reg.data_sovereignty` | unmeasured | ±18 | conditional | strong-candidate |
| Transmission line proximity <br>`pwr.transmission_proximity` | tier A | ±14 | conditional | viable |
| Organized opposition risk <br>`com.opposition_risk` | unmeasured | ±14 | conditional | viable |
| Modeled 10-year TCO <br>`eco.total_cost_of_ownership` | unmeasured | ±14 | conditional | viable |
| Residential proximity and noise exposure <br>`com.residential_proximity` | unmeasured | ±14 | conditional | viable |
| Construction labor availability <br>`eco.construction_labor` | unmeasured | ±14 | conditional | viable |
...
1. **[gate]** **Can the intended compute legally be installed and operated here?**
   - `reg.export_controls` · 18 points at risk · 3–10 weeks
   - **Ask:** Export-control counsel
...
2. **[gate]** **Is there active, organized opposition to data centers in this jurisdiction?**
   - `com.opposition_risk` · 14 points at risk · 2–6 weeks
   - **Ask:** Public record — planning commission and board minutes
...
3. **[gate]** **How much spare capacity (MVA) exists at the nearest suitable substation?**
   - `pwr.substation_headroom` · 10 points at risk · 8–26 weeks
...
4. **[gate]** **Realistic years from application to energization for target load.**
   - `pwr.interconnect_queue_time` · 10 points at risk · 4–12 weeks
...
5. **[gate]** **What fraction of available renewable supply is already withdrawn in this basin?**
   - `wtr.basin_stress` · 8 points at risk · 8–26 weeks
...
6. **[gate]** **Does the site intersect a modeled flood zone, and at what return period?**
   - `clm.flood_riverine` · 8 points at risk · 4–12 weeks
...
7. **[gate]** **How hard is it to obtain the legal right to the required volume?**
   - `wtr.withdrawal_permitting` · 8 points at risk · 10–30 weeks
   - **Ask:** State/provincial water regulator
   - **Get:** Permit precedent for comparable industrial withdrawal in the same basin
8. **[gate]** **Is the current zoning compatible, and if not, how hard is the path?**
   - `lnd.zoning_status` · 6 points at risk · 3–10 weeks
...
|---|---|---|---|---|
| `hyperscale_training` | **viable** *(capped by gate.compute_legally_deployable)* | 60 ± 19 | 0.14 | 19% |
| `inference_edge` | **viable** *(capped by gate.compute_legally_deployable)* | 60 ± 19 | 0.14 | 18% |
| `retrofit_colo` | **viable** *(capped by gate.compute_legally_deployable)* | 60 ± 19 | 0.13 | 18% |
...
- Sufficient contiguous developable land — 4163 ha contiguous developable area (computed upper bound).
...
| Contiguous developable area <br>`lnd.contiguous_area` | 4162.6 | hectares | 100 | 9 | C (researched) | `osm_overpass` | 2026-09-02 (1d) |
| Transmission line proximity <br>`pwr.transmission_proximity` | 9.46 | km | 70 | 9 | A (machine API) | `osm_overpass` | 2026-09-02 (1d) |
...
| On-site generation potential <br>`pwr.onsite_generation_potential` | 57.8 | index_0_100 | 58 | 8 | C (researched) | `composite` | 2026-09-02 (1d) |
...
| Dry-bulb temperature profile and free-cooling hours <br>`clm.dry_bulb_profile` | 5438.3 | free_cooling_hours_per_year | 70 | 7 | A (machine API) | `open_meteo_archive` | 2026-09-02 (1d) |
| Residential proximity and noise exposure <br>`com.residential_proximity` | — | dwellings_within_1km | — | 7 | — (unmeasured) | — | — |
| Long-haul fiber proximity <br>`cnx.longhaul_fiber_proximity` | 9.93 | km | 68 | 7 | D (modeled) | `osm_overpass` | 2026-09-02 (1d) |
...
| Wet-bulb temperature profile <br>`wtr.wetbulb_profile` | 103.4 | hours_above_24C_wetbulb | 92 | 7 | A (machine API) | `open_meteo_archive` | 2026-09-02 (1d) |
...
| Brownfield and retired-asset opportunity <br>`lnd.brownfield_opportunity` | light_industrial_reuse | category | 60 | 6 | C (researched) | `osm_overpass` | 2026-09-02 (1d) |
...
| Reclaimed water availability <br>`wtr.reclaimed_availability` | 12.17 | km_to_wwtp | 57 | 6 | B (bulk/structured) | `osm_overpass` | 2026-09-02 (1d) |
| Extreme heat trend to 2050 <br>`clm.extreme_heat_trend` | 2.13 | delta_C_design_day_2050 | 49 | 5 | D (modeled) | `open_meteo_archive` | 2026-09-02 (1d) |
...
| Internet exchange and cloud on-ramp proximity <br>`cnx.ixp_proximity` | 238.8 | km | 47 | 3 | A (machine API) | `peeringdb` | 2026-09-02 (1d) |
...
- **Residential proximity and noise exposure** (`com.residential_proximity`) — all Overpass mirrors failed: HTTP 504 from https://overpass.kumi.systems/api/interpreter
...
- **Port and rail access** (`eco.port_rail_access`) — all Overpass mirrors failed: HTTP 504 from https://overpass.kumi.systems/api/interpreter
...
- **Substation available capacity** (`pwr.substation_headroom`) — all Overpass mirrors failed: ReadTimeout: The read operation timed out
