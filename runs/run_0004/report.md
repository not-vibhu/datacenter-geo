# Site analysis — Jamnagar, Gujarat IN

**Jamnagar City Taluka, Gujarat, India** · `22.4700, 70.0600` · AOI radius 10 km
**Market:** DISCOM / Gujarat SERC
**Run:** `run_0004` · 2026-09-02T15:08:07Z
...
| **MAJOR** | Reclaimed water availability is unmeasured | 9 pts | Water utility or basin regulator |
| **MAJOR** | Industrial retail power price is unmeasured | 8 pts | Utility rate desk or wholesale market data provider |
| **MAJOR** | Blowdown and thermal discharge permitting is unmeasured | 8 pts | Water utility or basin regulator |
| | *… 23 more* | | |
...
36 factor(s) can move the score across a verdict threshold on their own. Each row is the real scorer re-run with that one factor resolved at its best and worst plausible value, everything else held.
...
| Port and rail access <br>`eco.port_rail_access` | tier B | ±14 | weak | conditional |
| Transmission line proximity <br>`pwr.transmission_proximity` | tier A | ±12 | conditional | viable |
| Organized opposition risk <br>`com.opposition_risk` | unmeasured | ±12 | conditional | viable |
| Residential proximity and noise exposure <br>`com.residential_proximity` | unmeasured | ±12 | conditional | viable |
...
5. **[gate]** **How much spare capacity (MVA) exists at the nearest suitable substation?**
...
6. **[gate]** **Realistic years from application to energization for target load.**
...
7. **[gate]** **Does the site intersect a modeled flood zone, and at what return period?**
...
|---|---|---|---|---|
| `hyperscale_training` | **conditional** | 58 ± 19 | 0.15 | 18% |
| `inference_edge` | **viable** *(capped by gate.compute_legally_deployable)* | 60 ± 19 | 0.14 | 17% |
| `retrofit_colo` | **viable** *(capped by gate.compute_legally_deployable)* | 60 ± 19 | 0.13 | 16% |
...
| Transmission line proximity <br>`pwr.transmission_proximity` | 18.57 | km | 48 | 9 | A (machine API) | `osm_overpass` | 2026-09-02 (1d) |
...
| On-site generation potential <br>`pwr.onsite_generation_potential` | 56.0 | index_0_100 | 56 | 8 | B (bulk/structured) | `composite` | 2026-09-02 (1d) |
...
| Dry-bulb temperature profile and free-cooling hours <br>`clm.dry_bulb_profile` | 2274.7 | free_cooling_hours_per_year | 32 | 7 | A (machine API) | `open_meteo_archive` | 2026-09-02 (1d) |
...
| Wet-bulb temperature profile <br>`wtr.wetbulb_profile` | 3850.9 | hours_above_24C_wetbulb | 1 | 7 | A (machine API) | `open_meteo_archive` | 2026-09-02 (1d) |
...
| Terrain slope <br>`lnd.slope` | 0.54 | percent_slope | 99 | 6 | A (machine API) | `opentopography` | 2026-09-02 (1d) |
...
| Extreme heat trend to 2050 <br>`clm.extreme_heat_trend` | 0.97 | delta_C_design_day_2050 | 83 | 5 | D (modeled) | `open_meteo_archive` | 2026-09-02 (1d) |
...
| Port and rail access <br>`eco.port_rail_access` | 2.34 | km_to_capable_port_or_rail | 99 | 4 | B (bulk/structured) | `osm_overpass` | 2026-09-02 (1d) |
...
| Internet exchange and cloud on-ramp proximity <br>`cnx.ixp_proximity` | 77.64 | km | 74 | 3 | A (machine API) | `peeringdb` | 2026-09-02 (1d) |
...
- **Long-haul fiber proximity** (`cnx.longhaul_fiber_proximity`) — no telecom routes or facilities mapped within 30 km. Absence in OSM is NOT evidence of absence — carriers do not publish routes. Requires carrier enquiry or a paid route dataset.
...
- **Substation available capacity** (`pwr.substation_headroom`) — OSM does not publish substation capacity. Found 77 substation(s) within 40 km (10 at >=110 kV); nearest qualifying is Naghedi at 2.74 km, 132.0 kV. Headroom must be obtained from the utility's IRP or transmission planning study.
...
- **water._adapter_reclaimed** (`water._adapter_reclaimed`) — adapter exceeded the 180s ceiling and was abandoned; the source is unresponsive from this network
...
- **Reclaimed water availability** (`wtr.reclaimed_availability`) — no adapter implemented; Tier C research required (sources: osm_wastewater, epa_cwns, municipal_utility_reports)
