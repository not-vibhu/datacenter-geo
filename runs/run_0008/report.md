# Site analysis — Mountain Road Tech Park, Hanover County VA (REJECTED 4-3, May 2026)

**Hanover County, Virginia, United States** · `37.6871, -77.5438` · AOI radius 10 km
**Market:** PJM
**Run:** `run_0008` · 2026-09-03T05:20:20Z
...
Not yet — and the reason is missing evidence, not a bad site. 7 knockout checks cannot be evaluated at all because the factor each one reads is unmeasured; 16 unmeasured factors could each move the verdict across a threshold on its own; only 27% of the decision weight is measured. Profitability in particular is unanswerable here: 5 of 5 cost inputs are unmeasured (Modeled 10-year TCO, Industrial retail power price, Large-load tariff availability, Land acquisition cost, Tax incentive package), so the score is a suitability screen, not a return. The verification queue is ordered by how much of the decision each answer settles.

*For profile `hyperscale_training`. Score 65 ± 17, confidence 0.23, 27% of decision weight measured.*
...
| **FATAL** | Water rights and withdrawal permitting is unmeasured, and Water viability for the assumed cooling architecture depends on it | 7 pts | State/provincial water regulator |
...
| **FATAL** | Contiguous developable area is unmeasured, and Sufficient contiguous developable land depends on it | 4 pts | Licensed surveyor / title company |
...
| **FATAL** | Protected area and habitat overlap is unmeasured, and Protected area exclusion depends on it | 3 pts | County or municipal planning department |
| **MAJOR** | Modeled 10-year TCO is unmeasured | 7 pts | Owner's cost consultant |
| **MAJOR** | Physical water supply availability is unmeasured | 7 pts | Water utility or basin regulator |
| **MAJOR** | Permitting timeline is unmeasured | 7 pts | Local land-use counsel |
| **MAJOR** | Construction labor availability is unmeasured | 7 pts | Owner's cost consultant / EPC |
| **MAJOR** | Critical equipment lead time is unmeasured | 7 pts | OEM / EPC procurement |
| **MAJOR** | Reclaimed water availability is unmeasured | 6 pts | Water utility or basin regulator |
| **MAJOR** | Environmental review burden is unmeasured | 6 pts | Local land-use counsel |
| | *… 19 more* | | |
...
This gate is a knockout check and its input is missing, so its result is not evidence of anything. Resolving the factor moves the score by up to 4 points on its own.
...
22 factor(s) can move the score across a verdict threshold on their own. Each row is the real scorer re-run with that one factor resolved at its best and worst plausible value, everything else held.
...
| Wet-bulb temperature profile <br>`wtr.wetbulb_profile` | tier A | ±14 | viable | strong-candidate |
| Port and rail access <br>`eco.port_rail_access` | tier B | ±10 | viable | strong-candidate |
| Transmission line proximity <br>`pwr.transmission_proximity` | tier A | ±9 | viable | strong-candidate |
| Baseline water stress <br>`wtr.basin_stress` | unmeasured | ±8 | viable | strong-candidate |
| Modeled 10-year TCO <br>`eco.total_cost_of_ownership` | unmeasured | ±7 | viable | strong-candidate |
| Physical water supply availability <br>`wtr.supply_availability` | unmeasured | ±7 | viable | strong-candidate |
| Water rights and withdrawal permitting <br>`wtr.withdrawal_permitting` | unmeasured | ±7 | viable | strong-candidate |
| Permitting timeline <br>`reg.permitting_timeline` | unmeasured | ±7 | viable | strong-candidate |
| Construction labor availability <br>`eco.construction_labor` | unmeasured | ±7 | viable | strong-candidate |
| Critical equipment lead time <br>`eco.equipment_lead_time` | unmeasured | ±7 | viable | strong-candidate |
| Reclaimed water availability <br>`wtr.reclaimed_availability` | unmeasured | ±6 | viable | strong-candidate |
...
1. **[gate]** **What fraction of available renewable supply is already withdrawn in this basin?**
...
2. **[gate]** **How hard is it to obtain the legal right to the required volume?**
   - `wtr.withdrawal_permitting` · 7 points at risk · 10–30 weeks
...
5. **[gate]** **Largest contiguous buildable area available at the site.**
   - `lnd.contiguous_area` · 4 points at risk · 4–12 weeks
   - **Ask:** Licensed surveyor / title company
   - **Get:** ALTA survey or local equivalent showing contiguous developable acreage
6. **[gate]** **Storm surge and 2050 sea level rise exposure.**
...
7. **[gate]** **Does the site intersect protected areas, wetlands, or critical habitat?**
   - `lnd.protected_overlap` · 3 points at risk · 4–16 weeks
   - **Ask:** County or municipal planning department
   - **Get:** Zoning determination letter and title/survey
8. **All-in modeled cost per kW-year across power, land, construction, tax and ops.**
...
|---|---|---|---|---|
| `hyperscale_training` | **viable** *(capped by gate.no_active_moratorium)* | 65 ± 17 | 0.23 | 27% |
| `inference_edge` | **viable** *(capped by gate.no_active_moratorium)* | 65 ± 17 | 0.23 | 28% |
| `retrofit_colo` | **viable** *(capped by gate.no_active_moratorium)* | 65 ± 17 | 0.22 | 26% |
...
- **Entitlement path exists** (caps score at 72) — Requires discretionary rezoning (rezoning_contested) — this converts an engineering problem into a political one.
...
- **No active data center moratorium** (caps score at 65) — Organized local opposition is present and has defeated or delayed comparable applications.
...
| Transmission line proximity <br>`pwr.transmission_proximity` | 2.68 | km | 93 | 9 | A (machine API) | `osm_overpass` | 2026-09-03 (1d) |
| Industrial retail power price <br>`pwr.retail_power_price` | — | USD/MWh | — | 9 | — (unmeasured) | — | — |
| Accelerator export control exposure <br>`reg.export_controls` | unrestricted | category | 100 | 9 | B (bulk/structured) | `bis_export_controls` | 2026-09-03 (1d) |
| Riverine and pluvial flood exposure <br>`clm.flood_riverine` | 1000 | return_period_years | 96 | 8 | A (machine API) | `fema_nfhl` | 2026-09-03 (1d) |
| Organized opposition risk <br>`com.opposition_risk` | organized_opposition | category | 20 | 8 | C (researched) | `local_news_media` | 2026-09-03 (1d) |
| Modeled 10-year TCO <br>`eco.total_cost_of_ownership` | — | USD_per_kW_year | — | 8 | — (unmeasured) | — | — |
| Zoning and entitlement status <br>`lnd.zoning_status` | rezoning_contested | category | 28 | 8 | C (researched) | `us_local_government` | 2026-09-03 (1d) |
| Large-load tariff availability <br>`pwr.large_load_tariff` | — | category | — | 8 | — (unmeasured) | — | — |
| On-site generation potential <br>`pwr.onsite_generation_potential` | 39.3 | index_0_100 | 39 | 8 | C (researched) | `composite` | 2026-09-03 (1d) |
...
| Dry-bulb temperature profile and free-cooling hours <br>`clm.dry_bulb_profile` | 6940.8 | free_cooling_hours_per_year | 86 | 7 | A (machine API) | `open_meteo_archive` | 2026-09-03 (1d) |
| Residential proximity and noise exposure <br>`com.residential_proximity` | 37 | dwellings_within_1km | 83 | 7 | A (machine API) | `osm_overpass` | 2026-09-03 (1d) |
| Long-haul fiber proximity <br>`cnx.longhaul_fiber_proximity` | — | km | — | 7 | — (unmeasured) | — | — |
...
| Wet-bulb temperature profile <br>`wtr.wetbulb_profile` | 347.5 | hours_above_24C_wetbulb | 78 | 7 | A (machine API) | `open_meteo_archive` | 2026-09-03 (1d) |
...
| Terrain slope <br>`lnd.slope` | 0.99 | percent_slope | 98 | 6 | A (machine API) | `opentopography` | 2026-09-03 (1d) |
| Protected area and habitat overlap <br>`lnd.protected_overlap` | — | percent_overlap | — | 6 | — (unmeasured) | — | — |
| Brownfield and retired-asset opportunity <br>`lnd.brownfield_opportunity` | heavy_industrial_reuse | category | 82 | 6 | C (researched) | `osm_overpass` | 2026-09-03 (1d) |
...
| Extreme heat trend to 2050 <br>`clm.extreme_heat_trend` | 1.69 | delta_C_design_day_2050 | 62 | 5 | D (modeled) | `open_meteo_archive` | 2026-09-03 (1d) |
...
| Carrier availability <br>`cnx.carrier_count` | 38 | count_carriers | 100 | 4 | B (bulk/structured) | `peeringdb` | 2026-09-03 (1d) |
| Port and rail access <br>`eco.port_rail_access` | 4.21 | km_to_capable_port_or_rail | 99 | 4 | B (bulk/structured) | `osm_overpass` | 2026-09-03 (1d) |
...
| Internet exchange and cloud on-ramp proximity <br>`cnx.ixp_proximity` | 8.58 | km | 96 | 3 | A (machine API) | `peeringdb` | 2026-09-03 (1d) |
...
- **Long-haul fiber proximity** (`cnx.longhaul_fiber_proximity`) — all Overpass mirrors failed: HTTP 504 from https://overpass.kumi.systems/api/interpreter
...
- **Substation available capacity** (`pwr.substation_headroom`) — all Overpass mirrors failed: HTTP 504 from https://overpass.kumi.systems/api/interpreter
...
- **Reclaimed water availability** (`wtr.reclaimed_availability`) — all Overpass mirrors failed: HTTP 504 from https://overpass.kumi.systems/api/interpreter
