...
/* The separability test uses the same rule as dcgeo.compare: a pair whose scores
   have overlapping score ± band intervals is not separable, and an edge only
...
function renderCompare() {
  ...
  for (let i = 0; i + 1 < ranked.length; i++) {
    ...
    if (Math.abs(a.p.score - b.p.score) <= a.p.band + b.p.band)
  ...
  $("panel").innerHTML = `
    ...
    <div class="cmp">
      <div class="cmpbanner ${separable ? "yes" : "no"}">${separable
        ? "Every adjacent pair has disjoint score ± band intervals, so this ordering is supported by the evidence."
